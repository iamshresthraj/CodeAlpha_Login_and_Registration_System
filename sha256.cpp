...
void SHA256::final(unsigned char *digest) {
    ...
    uint64 len_b;
    m_tot_len += m_len;
    pm_len = m_len < 56 ? (56 - m_len) : (120 - m_len);
    memset(&m_block[m_len], 0, pm_len);
    m_block[m_len] = 0x80;
    len_b = (uint64)m_tot_len * 8;
    
    // Store 64-bit length in big-endian format
    m_block[m_len + pm_len] = (unsigned char)((len_b >> 56) & 0xFF);
    m_block[m_len + pm_len + 1] = (unsigned char)((len_b >> 48) & 0xFF);
    m_block[m_len + pm_len + 2] = (unsigned char)((len_b >> 40) & 0xFF);
    m_block[m_len + pm_len + 3] = (unsigned char)((len_b >> 32) & 0xFF);
    m_block[m_len + pm_len + 4] = (unsigned char)((len_b >> 24) & 0xFF);
    m_block[m_len + pm_len + 5] = (unsigned char)((len_b >> 16) & 0xFF);
    m_block[m_len + pm_len + 6] = (unsigned char)((len_b >> 8) & 0xFF);
    m_block[m_len + pm_len + 7] = (unsigned char)(len_b & 0xFF);
    
    transform(m_block, m_len < 56 ? 1 : 2);
    for (int i = 0 ; i < 8; i++) {
